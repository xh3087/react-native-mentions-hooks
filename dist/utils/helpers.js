 ...
 */
const getTriggerPartSuggestionKeywords = (mentionState, selection, triggersConfig, onChange = () => { }, handleSelectionChange) => {
        ...
        keywordByTrigger[triggerName] = {
            ...
            onSelect: (suggestion) => {
                ...
                if (handleSelectionChange) {
                    const trueText = replaceTriggerValues(newValue, ({original, trigger, name, id})=>`@${name}`);
                    const oldText = mentionState?.plainText
                    const newSelection = {
                        nativeEvent: {
                            selection: {
                                start: selection.start+(trueText.length - oldText.length),
                                end: selection.end+(trueText.length - oldText.length)
                            }
                        }
                    }
